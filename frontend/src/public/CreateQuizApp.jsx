  ...
  return (
    <main className="mx-auto grid min-h-screen max-w-5xl place-items-center px-4 py-12">
      <section className="w-full max-w-2xl rounded-[2rem] border border-slate-200 bg-white p-8 text-center shadow-xl shadow-slate-200/70 dark:border-slate-800 dark:bg-slate-900 dark:shadow-slate-950/50 md:p-12">
        ...
        <p className="mx-auto mt-4 max-w-xl leading-7 text-slate-600 dark:text-slate-300">ログイン後、問題と選択肢を入力して自分だけのクイズを作成できます。</p>
        <div className="mt-8 flex flex-col justify-center gap-3 sm:flex-row">
          <a href="/login" className="rounded-2xl bg-slate-950 px-6 py-3 font-medium text-white hover:bg-slate-800 dark:bg-white dark:text-slate-950 dark:hover:bg-slate-200">ログインする</a>
          <a href="/signup" className="rounded-2xl border border-slate-300 bg-white px-6 py-3 font-medium hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-900 dark:hover:bg-slate-800">アカウントを作る</a>
        </div>
  ...
  const removeChoice = (questionIndex, choiceIndex) => {
    setDraft((current) => ({
      ...
      questions: current.questions.map((question, index) => {
        ...
        if (removedWasCorrect && choices.length > 0) choices[0] = { ...choices[0], isCorrect: true }
        ...
        <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_280px]">
          ...
          <aside className="lg:sticky lg:top-24 lg:self-start">
            <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-xl shadow-slate-200/60 dark:border-slate-800 dark:bg-slate-900 dark:shadow-slate-950/50">
              ...
              <a href="/quizzes" className="mt-3 block rounded-2xl px-5 py-3 text-center text-sm text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800">作成をやめる</a>
            </section>
          </aside>
        </div>
      </form>
